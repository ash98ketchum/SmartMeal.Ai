...
import { Bell, User, ShieldCheck, Database, BarChart3, Mail } from 'lucide-react';
        ...
        <SettingSection title="AI Model" icon={<BarChart3 size={20} />} index={1}>
          <div className="space-y-3">
            <ToggleSwitch
              ...
              label="Automatically update AI model when new versions are available"
            />
            <ToggleSwitch
              enabled={modelSettings.highPrecisionMode}
              onChange={() => handleModelSettingChange('highPrecisionMode')}
              label="High precision mode (may increase processing time)"
            />
            <ToggleSwitch
              enabled={modelSettings.saveIngredientData}
              onChange={() => handleModelSettingChange('saveIngredientData')}
              label="Save ingredient data for improved future predictions"
            />
            
            <div className="pt-2">
              <Button variant="cyan">Recalibrate Model</Button>
            </div>
          </div>
        </SettingSection>
