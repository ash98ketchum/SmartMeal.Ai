...
def write_model(agent: EpsilonGreedyAgent, path: str):
    """Serialize the trained agent to a pickle file."""
    ...
    print(f"[{datetime.now()}] Model saved to {path}")

def main():
    df = load_history(DATA_PATH)
    agent = train_bandit(df)
    write_model(agent, MODEL_PATH)
